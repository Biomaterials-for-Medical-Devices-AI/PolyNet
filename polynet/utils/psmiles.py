# code taken directly from the PSmiles repository. We will use the original implmentation once it becomes available again.
...
class PolymerSmiles:
    def __init__(self, psmiles: str, deactivate_warnings: bool = False):
        ...
        if not self.ladder:
            ...
            if m is None:
                ...
                if error:
                    raise UserWarning(f"Invalid chemistry of {self.psmiles}. Issue with {error}")
        ...
        if self.ladder:
            logging.warning(
                "Ladder polymer detected. Only PG fingerprints are " "tested for ladder polymers."
            )
    ...
    def get_connection_info(self, mol: Chem.RWMol = None, symbol: str = "*") -> Dict:
        ...
        if neighbor_indices[0][0] != neighbor_indices[1][0]:
            s_path = Chem.GetShortestPath(mol, neighbor_indices[0][0], neighbor_indices[1][0])
    ...
    def replace_stars(self, _with: str) -> PolymerSmiles:
        ...
        """
        return PolymerSmiles(self.psmiles.replace("[*]", _with), deactivate_warnings=True)
    ...
    def randomize(self) -> PolymerSmiles:
        ...
        """
        sm = Chem.MolToSmiles(Chem.MolFromSmiles(self.psmiles), doRandom=True, canonical=False)
        sm = sm.replace("*", "[*]")
    ...
    def dimer(self, how: int = 1) -> PolymerSmiles:
        ...
        connect = [info["star"]["index"][0], info["star"]["index"][how] + len(info["symbols"])]

        logging.debug(f"(3) Connect star atoms {connect[0]} and {connect[1]} with {bond_type = }")
    ...
    def fingerprint_mordred(self) -> Dict[str, float]:
        ...
        dim = calc.pandas([can_smiles.dimer().replace_stars("[At]").mol], quiet=True, nproc=1)
    ...
    def alternating_copolymer(
        ...
        ed.AddBond(idx_star1[how[0]], idx_star2[how[1]] + len(symbols1), order=bond_type)
    ...
    def linear_copolymer(
        self, other: Union[PolymerSmiles, str], pattern: Union[str, List[int]] = [0, 0, 0, 1, 1, 1]
    ) -> PolymerSmiles:
    ...
    def random_copolymer(
        self, other: Union[PolymerSmiles, str], ratio: float = 0.5, units: int = 10
    ) -> PolymerSmiles:
