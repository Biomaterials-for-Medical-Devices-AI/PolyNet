...
def get_connection_info(mol=None, symbol="*") -> Dict:
    ...
    ret_dict["star"] = {"index": stars_indices, "atom_type": stars_type, "bond_type": stars_bond}
    ...
    neighbor_bonds = [
        [mol.GetBondBetweenAtoms(stars_indices[0], x).GetBondType() for x in neighbor_indices[0]],
        [mol.GetBondBetweenAtoms(stars_indices[1], x).GetBondType() for x in neighbor_indices[1]],
    ]
    s_path = None
...
def get_index_to_break(mol: Chem.RWMol, info: Dict) -> Tuple[int, int]:
    ...
    for atom_ring, bond_ring in zip(new_atom_rings, new_bond_rings):
        if set(atom_ring).issuperset(set(n_idx)) and len(atom_ring) == ring_length_to_break:
    ...
    def get_break_idx(ring_to_break):
        ...
        for idx_1 in sorted_ring:
            ...
            possible_neighbors = [x.GetIdx() for x in mol.GetAtomWithIdx(int(idx_1)).GetNeighbors()]
